...
class NPS(ArrayWithBenefits):
    ...
    @classmethod
    def from_dh(cls, dh, group: str = "noise", dataset: list = "nps"):
        """
    ...
    def to_dh(self, dh, group: str = "noise", dataset: list = ["nps", "cps"], **kwargs):
        """
        ...
        data_cps = self._cps if self._n_channels > 1 else None
        dh.set(group, **{dataset[0]: data_nps}, **kwargs)
        if data_cps is not None:
            dh.set(group, **{dataset[0]: data_cps}, **kwargs)
